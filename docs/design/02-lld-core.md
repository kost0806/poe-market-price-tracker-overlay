...
> **실증 (2026-08-16, 한글 사전).** 968개 슬러그와 `ui.*` 문구를 한 파일(`ko.json`)에 담아 그대로 동작했다 — 슬러그는 케밥 표기라 `ui.`로 시작할 수 없으므로 두 이름 공간이 충돌하지 않는다는 이 절의 전제가 실제 데이터에서 성립한다. 사전의 출처와 생성 절차는 `00-api-contract.md` §6, 파일 배치는 S4 §2.1·§2.5.
>
> **다만 "파일 추가만으로"가 참인 범위는 이 모듈까지다.** 같은 작업에서 설정 창이 이 구조를 아예 쓰지 않고 XAML에 영문을 적어 두었음이 드러났다(S3 §5.4.4). 사전은 그 자리에 도달할 수 없었고, 도달할 표면을 만드는 데 코드가 필요했다.
...
**`en` 외의 사전은 내장하지 않는다.** `ko.json`은 출력 디렉터리로 복사만 된다(S4 §2.1). 바닥은 하나여야 하고, 두 번째 내장 사전은 층을 늘리는 것이 아니라 ③의 유일성을 흐린다 — 체인이 ③에서 답을 얻지 못하는 경우는 정의상 없으므로 다른 언어를 내장해도 도달하지 않는다.
