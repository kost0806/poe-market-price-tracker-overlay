...
| M-9 | **`PeriodicTimer.Period`는 대기 중인 틱에 소급 적용된다** (3000ms 대기 중 150ms로 변경 → 156ms에 발화) | §7.7의 【확인】 종결. PL18 기대값 정정 |
...
- **【측정】 `PeriodicTimer.Period` 변경은 이미 대기 중인 틱에 소급 적용된다** (3000ms 대기 중 150ms로 바꾸자 156ms에 발화). 초판의 【확인】을 종결한다. **함의**: 5분 → 30분 변경이 진행 중인 대기를 **25분 늘린다.** 사용자는 주기를 바꾼 직후 화면이 30분간 멈춘 것을 본다. 주기를 **늘릴 때는** 변경 직후 1회 라운드를 돌릴지 S3와 함께 판단한다 → §12-31.
...
| **PL18** | `interval` 5 → 30, 이미 3분 대기 중 | 두 카운터 불변, 라운드 취소 없음, **다음 틱은 시작으로부터 30분 뒤** — 【측정】 `Period`가 소급 적용된다 |
...
| 31 | **주기를 늘리면 대기 중인 라운드가 그만큼 연장된다** | 【측정】 파생 | `Period`는 소급 적용된다. 5분 → 30분 변경 직후 화면이 30분 멈춘다. 변경 직후 1회 라운드를 돌릴지 S3와 함께 판단 |
