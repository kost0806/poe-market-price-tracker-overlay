...
| | |
|---|---|
| 문서 상태 | **실측 확정. 반영 완료 (2026-08-16)** — §7의 개정과 후속 작업이 전부 끝났다. 이 문서는 이제 실측 기록으로만 남는다 |
| 측정일 | 2026-08-16 |
| 측정 리그 | `Allflame` |
...
### 그다음에 일어난 일 (2026-08-16, 같은 날)

이 문서를 쓴 시점에는 `src/`를 한 줄도 건드리지 않은 상태였다. 그 뒤 §7의 순서대로 진행했고 **전부 끝났다.**

| §7 항목 | 결과 |
|---|---|
| 1. `00-api-contract.md` 신규 절 | **§6으로 들어갔다.** 이 문서 §1~§5·§8·§9가 그 절의 원본이다 |
| 2. `REQUIREMENTS.md` | §3 주의 보강, FR-07-3 개정, §11 항목 해소, 결정 **D9** 신설 |
| 3. `02-lld-core.md` / `04-dld.md` | 구조 변경 없음을 확인. `ko.json` 배치(S4 §2.1)와 생성기(S4 §2.5)만 추가 |
| 4. `ui.*` 한글 문구 | **40개가 아니라 109개였다** — 아래 참조 |
| 5. 카테고리 라벨 18개 | 17개 작성. `DjinnCoin`은 KR 출처에 아예 없어 **비워 두었다**(폴백이 영문을 보인다) |
| 6. `ko.json` | 1,077키(아이템 968 + `ui.*` 109). `tools/build-ko-dictionary.py`가 생성한다 |
| 7. 생성기 배치 | `tools/`에 두되 **빌드에 걸지 않는다**. 산출물을 커밋한다 |

**§7이 「약 40개」라 적은 것은 틀렸다.** `en.json`의 `ui.*` 키는 그 시점에 이미 57개였고, 세어 보니 **설정 창이 자기 문구를 사전에 넣은 적이 없었다** — XAML에 영문 리터럴 32개가 그대로 있었고 `ui.settings.*` 키는 존재한 적이 없다. FR-07-1 위반이며, 그것을 닫으면서 키가 110개가 됐다. 경위와 처분은 `03-lld-shell.md` §0.6·§5.4.4에 있다.

**「사전만 얹으면 된다」(요구사항 D5)는 이 모듈에서는 참이었고 설정 창에서는 거짓이었다.** 구조를 1일차부터 둔 것과 모든 사용처가 그 구조를 쓰는 것은 다른 이야기다.
...
## 7. 다음 단계 — 문서가 먼저다  【전부 완료. §0의 표가 결과다】

> 아래는 작성 시점의 계획이며 **기록을 위해 원문 그대로 둔다.** 실제 결과는 §0의 표에 있고, 4번 항목의 「약 40개」는 틀린 추정이었다.
