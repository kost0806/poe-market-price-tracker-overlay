...
### 4.8 ~~시세 행의 변동률 칸~~ — **칸이 사라졌다** (REQUIREMENTS 4판 FR-04-1, HLD D25)

이 절은 화면 실측이 잡은 결함(`▼ ▼5.0%` — 글리프가 두 번 그려졌다)과 그 처방(뷰는 `Change.Text` 하나만 바인딩한다), 그리고 `ChangeDirection`별 잠정 팔레트를 담고 있었다. **변동률 칸 자체가 없어졌으므로 셋 다 대상이 없다.**

근거는 `00-api-contract.md` §3.2의 실측이다 — 창이 약 7일이고, 기준 통화가 `maxVolumeCurrency`라 표시 통화를 고정한 행에서는 부호가 갈린다. **화면의 색이 반대로 칠해질 수 있었다는 뜻이다**: 이 절이 정한 상승색·하락색은 문제를 더 또렷하게 만드는 쪽으로 작동했다.

**되살릴 때를 위해 남기는 두 가지.**

- 결함의 성격: `ChangeDisplay.Text`가 **이미 글리프를 포함**했고(`ui.price.change` = `{0}{1}%`), 뷰가 `Glyph`와 `Text`를 인접한 두 `Run`으로 나란히 놓아 같은 화살표가 두 번 그려졌다. 「필드가 둘 있으니 둘 다 그린다」가 원인이다.
- 잠정 팔레트: `Up` `#FF7FD18C` · `Down` `#FFE58A8A` · 그 외 `#FFA0A0A0`. §14-12의 판독성 실험 대상 목록에서는 이 항목이 빠진다.
...
**칸은 셋이다.** 4판까지 있던 네 번째 칸(변동률)은 REQUIREMENTS 4판 FR-04-1로 사라졌다(§4.8).
