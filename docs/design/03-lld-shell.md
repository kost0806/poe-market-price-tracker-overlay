...
### 0.6 제7판 — 한글 사전을 얹으려다 설정 창이 FR-07-1을 전혀 지키지 않았음이 드러났다. §5.4.4 신설, §11 정정

한글 사전(`00-api-contract.md` §6)을 얹는 작업에는 **설계상 코드 변경이 필요 없어야 했다**(FR-07-3). 사전을 넣기 전에 사용처를 세어 보니 그 전제가 설정 창에서만 성립하지 않았다.

| # | 항목 | 처분 |
|---|---|---|
| E11 | **`SettingsWindow.xaml`에 사용자 노출 영문 리터럴이 32개 있다.** `ui.settings.*` 키는 존재한 적이 없다 | **FR-07-1 위반.** §12는 이미 FR-07-1의 충족처로 "§5.4/§11(두 창 View)"를 지목하고 있었다 — 설계가 요구한 것을 구현이 빠뜨렸고, 두 문서 어디에도 그 32개가 어디서 오는지 적혀 있지 않았다. **§5.4.4 신설**로 소재를 확정한다 |
| E12 | **`SettingsViewModel`이 `ILocalizer.LanguageChanged`를 구독하지 않는다** | **§11 D-PS5 위반.** 세 뷰모델 중 둘(`Overlay`·`Tray`)만 구독하고 있었다. 30초 팬아웃 패스가 `RunSearch`를 다시 돌려 검색 행만 우연히 따라왔을 뿐, 라벨은 애초에 재계산할 대상이 없었다(E11이 그 이유다). §11에 실제 재계산 목록을 명시한다 |
| E13 | 열거형을 그대로 그리는 자리가 둘 더 남아 있다 — `LeaguesStatus`, 검색 결과의 `Category` | §5.4.3이 이미 "사용자에게 내부 토큰을 보이지 않는다"로 닫은 것과 같은 결함이다. 그 표에 두 행을 추가한다 |
| E14 | 관심목록 행이 **아이템 슬러그**(`Id.Value`)를 그린다 | 오버레이·검색 결과는 `DisplayName`으로 고쳤으면서(D-DL16) 관심목록만 남았다. 사전을 얹으면 이 목록만 영문 슬러그로 남는다 — §5.4.3 표에 등재하고 같은 ③④⑤ 체인을 태운다 |

**이 판이 만든 것은 없다.** 넷 다 이미 있던 결함이며, 한글 사전은 그것을 **드러낸** 계기다. FR-07-3의 "코드 변경 없이"는 사전 파일에 대한 조건으로서 여전히 참이다.
...
| **검색 결과의 카테고리 열**(E13, 제7판) | `ExchangeCategory`를 그대로 바인딩 — `DivinationCard`·`AllflameEmber` 같은 파스칼 식별자가 그대로 보인다 | `ui.category.*` 18개 키로 조회한다(S4 §14.10). 열거형 이름은 poe.ninja의 질의 타입이지 사람이 읽을 이름이 아니며, 어떤 사전으로도 번역되지 않는다. `SearchRowViewModel.CategoryLabel`이 소재다 |
| **리그 목록 상태**(E13, 제7판) | `LeaguesStatus`를 그대로 바인딩 — `Ok`/`Suspicious`/`Failed` | 같은 이유로 세 문장으로 바꾼다(`ui.settings.leagueStatus.*`). 특히 `Suspicious`는 그 단어만으로 무엇이 의심스러운지 말하지 않는다 — D6이 그 값을 만든 이유(첫 원소가 `Standard`/`Hardcore`)를 문장이 담는다 |
| **관심목록 행**(E14, 제7판) | `Id.Value` — 슬러그 그대로(`abandoned-wealth`) | `ItemName` ③④⑤ 체인을 태운다. 검색 결과 행을 D-DL16으로 고칠 때 같이 고쳤어야 했던 자리다. `apiName`은 `WatchlistEntry`에 없으므로 `null`을 넘긴다 — 사전에 없으면 ⑤로 떨어져 **오늘과 같은 슬러그**가 보이므로 잃는 것이 없고, 사전에 있으면 이름이 보인다 |

**5.4.4 설정 창의 고정 문구는 어디서 오는가 — 신규 D-SH23 (E11, 제7판)**

§12는 FR-07-1의 충족처로 이 절을 지목해 두었지만, **이 창의 라벨·머리글·버튼 문구가 어느 표면에서 오는지는 어느 문서에도 없었다.** 그래서 구현은 XAML에 영문을 직접 적었고, 32개 문자열이 어떤 사전으로도 번역되지 않는 상태로 동결됐다.

**결정: 뷰모델이 조립한 하나의 문자열 묶음을 뷰가 바인딩한다.**

| 후보 | 판정 |
|---|---|
| XAML에서 마크업 확장(`{loc:Ui ui.settings.league}`)으로 직접 조회 | **기각.** 언어 전환 시 재평가를 스스로 만들어야 하고, D-PS5가 이미 "뷰모델이 재계산한다"로 그 책임을 배정했다. 두 번째 재계산 경로가 생긴다 |
| 문자열 32개를 `SettingsViewModel`의 속성 32개로 | **기각.** 언어 전환마다 알림 32건이 나가고, 뷰모델의 표면이 상태와 문구로 뒤섞인다 |
| **`SettingsViewModel.Strings` — 조회 결과만 담은 불변 묶음 하나** | **채택.** 언어 전환은 묶음을 통째로 갈아 끼우고 알림 **한 건**을 낸다. 뷰는 `{Binding Strings.League}`로 읽는다 |

32개 중 **29개**가 이 묶음의 고정 라벨이고, 나머지 셋(검색 결과 상태 문장)은 `SearchOutcome`에 따라 갈리므로 뷰모델의 계산 속성 `SearchStatusText` 하나로 합친다 — XAML의 `DataTrigger` 세 개가 사라진다. 여기에 `LeagueStatusText`(E13)가 같은 방식으로 하나 더 붙는다. 키 카탈로그는 **`ui.settings.*` 35개**다(S4 §14.9): 고정 29 + 검색 상태 3 + 리그 상태 3.

**`Shell`은 키 상수를 볼 수 없다.** `UiStateKeys`·`UiStateTemplates`는 `Core`의 `internal`이고 `InternalsVisibleTo`는 테스트 프로젝트에만 열려 있다 — `SettingsWindowFactory`가 `ui.footer.attribution`을 **문자열 리터럴로** 적고 있는 이유가 이것이다. 따라서 조립은 `Core`(=`Presentation`) 쪽에 있어야 하며, 이 결정은 그 제약의 결과이기도 하다.

**출처 표기도 이 묶음으로 옮긴다.** 지금은 `SettingsWindowFactory`가 생성자 인자로 넘기고 코드비하인드가 `Attribution.Text`에 한 번 쓴다 — 창이 열려 있는 동안 언어를 바꾸면 그 줄만 옛 언어로 남는다. 묶음에 넣으면 다른 29개와 같은 경로로 갱신되고, 생성자에서 인자 하나가 사라진다(S4 §12.6 개정). 키는 새로 만들지 않는다 — 오버레이 푸터와 같은 `ui.footer.attribution`을 계속 쓴다(§5.4 표가 이미 그렇게 정해 두었다).

**상수 폴백은 두지 않는다.** §9.3이 상수를 요구한 대상은 "자리표시자를 가진 키"와 "사용자가 반드시 읽어야 하는 상태 배너·행"이다(S4 §14 D1 규칙). `ui.settings.*` 35개는 전부 자리표시자가 없고, 체인이 ⑤까지 떨어져도 `ui.settings.league`라는 키가 그 자리에 보일 뿐이다 — 진단으로 읽히며 기능이 사라지지 않는다. 트레이 메뉴 항목과 같은 취급이다. `ui.category.*` 18개도 같다.

### 5.4.6 검색 — 카탈로그와 캐시를 병합한다 (신규 D-SH24, FR-01-1 / HLD D7 개정)

`SettingsViewModel`이 두 출처를 각각 뒤지고 하나의 목록으로 낸다. `Store`는 캐시만 알고, 카탈로그(`02-lld-core.md` §6.8)는 이 뷰모델이 안다.

| 단계 | |
|---|---|
| ① 캐시 | `_searchSource.Search(query, new SearchOptions(limit, MatchesLocalisedName))` — 지금 그대로다 |
| ② 카탈로그 | `ItemCatalog.Entries`를 훑어 **슬러그·영문 이름·지역화 이름** 셋 중 하나라도 걸리면 적중. ①이 이미 낸 `ItemId`는 건너뛴다 |
| ③ 정렬 | **시세가 있는 적중이 먼저**, 그 안에서 기존 순위(완전일치 → 접두 → 부분). 값이 붙은 행이 사용자가 대개 찾던 것이다 |
| ④ 상한 | 병합 뒤에 `SearchLimit`으로 자른다. ①이 상한을 채워도 ②가 잘릴 뿐 ①을 밀어내지 않는다 |

**행의 표시.** `SearchRowViewModel`이 `PriceText`를 하나 더 나른다 — 캐시에서 온 행은 지금까지처럼 이름과 카테고리만 쓰고, 카탈로그에서만 온 행은 그 자리에 `ui.settings.search.noPrice`(`시세 없음`)를 흐린 색으로 적는다. **행을 감추거나 비활성화하지 않는다**: 관심목록 추가가 바로 그 행에서 성립하고(추가가 그 카테고리를 1회 조회한다 — 이미 있던 경로), 못 쓰는 것처럼 그리면 사용자가 누를 이유를 잃는다.

**아직 받지 않은 카테고리 버튼 목록은 유지한다.** 검색이 카탈로그를 덮은 뒤에도 "이 카테고리 전체를 미리 받아 둔다"는 조작은 남는다 — 다만 이제 그것이 **검색을 성립시키는 유일한 길이 아니다.**
...
> **E12 정정(제7판).** 구현은 셋 중 둘만 구독했다 — `SettingsViewModel`에 구독이 없었다. 증상이 오래 가려져 있었던 이유는 두 가지다. ① 30초 팬아웃 패스가 `Refresh` → `RunSearch`를 다시 돌려 **검색 행만** 우연히 따라왔고, ② 라벨은 애초에 XAML 리터럴이라 재계산할 대상 자체가 없었다(E11). §5.4.4가 `Strings`를 만들면 ②가 사라지므로 구독은 선택이 아니다. **이 뷰모델은 셋 중 유일하게 일회성(창 수명)이므로 `Dispose`에서 반드시 해지한다** — 다른 둘은 앱 수명이라 해지하지 않는다.

| 뷰모델 | 재계산 범위 |
|---|---|
| `OverlayViewModel` | 모든 행의 `Pricing.Format`/`Change`/`Relative` 재호출(숫자 자체는 불변, 템플릿과 상대 시각 단위만 바뀐다), 배너 문구, 푸터 |
| `SettingsViewModel` | `Strings` 묶음 통째(§5.4.4 — 라벨·머리글·버튼·출처 표기), 검색 상태 문장, 검색 결과의 아이템 이름과 카테고리 라벨, 관심목록 행 이름, 조회 안 된 카테고리 버튼. `ItemName` 폴백 체인이 언어에 의존하므로(S2 §3.4) 이름들은 다시 풀어야 한다 |
...
| **FR-07-1** | **모든 표시 문자열은 ID로 조회 — 전 사용처, 트레이 툴팁·메뉴 포함 — §5 항목1, 신규 등재** | §6.3(트레이 툴팁·메뉴 조립), §5.4/§11(두 창 View), **§5.4.4(설정 창 고정 문구의 소재 — D-SH23, 제7판. 이 행이 지목하던 "§5.4"에는 실제로 그 소재가 적혀 있지 않았다)**, §9.3(`ui.state.*`/`ui.tray.*` 상수 폴백 상속). **HLD §8의 FR-07-1 행은 단계를 "S2"로만 적었으나 범위가 Shell에도 착지한다 — 개정 요구로 "S2+S3"로 정정한다(§13)** |
| FR-07-4 | UI 문구 ID 조회 (트레이 포함) | §6.3, §11, §9.3(M7, `ui.state.*`/`ui.tray.*` 상수 폴백 상속), **§5.4.4(설정 창) + §5.4.3의 카테고리·리그 상태 행(E13, 제7판)** |
