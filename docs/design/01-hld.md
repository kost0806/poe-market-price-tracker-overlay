...
### 0.1 개정 4판 — 측정으로 확정하고, 표시 전용화가 남긴 구멍을 메웠다
...
| 5 | `settingsWindow.Owner = MainWindow` 시 exstyle `0x00040108` — `WS_EX_TOPMOST` 켜짐. 그런데 `Topmost` 속성은 false로 읽힘 | **설정 창에 `Owner`를 지정하지 않는다**를 불변식화(D18-b). 소스 어디에도 `Topmost=true`가 없는데 설정 창이 게임 위로 뜨는 사고 방지 |
...
graph LR
    ...
    APP -->|설정 창 HWND, 일반 활성화 가능, Owner 미지정| DWM
...
유휴 상태에서 도는 타이머는 둘뿐이다. 애니메이션·주기 리페인트 금지, 브러시·지오메트리는 `Freeze()`. **UI 가상화는 도입하지 않는다** — 오버레이는 스크롤 자체가 없고 설정 창 목록은 열려 있는 동안만 존재한다.
...
[STAThread] static void Main
 ...
 9. 오버레이 창 생성
      WindowStyle=None, AllowsTransparency=True, ShowActivated=False, ShowInTaskbar=False,
      Topmost=True, ResizeMode=NoResize
      SourceInitialized 에서 TRANSPARENT 와 NOACTIVATE 를 읽고-고쳐-쓰기로 설정  (D4-d)
      저장된 기하 검증 후 적용 — 작업영역에 완전히 포함되지 않으면 기본 위치  (FR-05-4, §7)
      높이 정책 적용 (D19)
...
11. app.Run(overlayWindow)
      ※ MainWindow 지정은 편의이며 함정이다. 【측정】 설정 창의 Owner 로 쓰면
        WS_EX_TOPMOST 가 전파되어 Topmost 를 켠 적 없는데 게임 위로 뜬다. Owner 금지 (D18-b)
...
- 유령 아이콘은 프로세스 급사에서 생기지, 수백 밀리초의 순서 차이에서 생기지 않는다. 그래서 폐기를 `StopAsync` 뒤로 미뤄도 D18-c의 목적은 유지된다.
...
sequenceDiagram
    ...
    U->>Tray: 아이콘 클릭
    Tray->>SV: 설정 창 생성 및 표시, Owner 지정 없음
    U->>SV: 검색어 입력
...
|---|---|---|
| `Topmost` | 상시 | FR-05-1 |
| `WindowStyle` | **`None`** | `AllowsTransparency=true`는 이것 없이 예외를 던진다. 런타임 변경 금지(D4-d) |
| `AllowsTransparency` | `True` | 배경 투명·둥근 모서리 |
...
| 잔여 조건 | ① **완전 투명 픽셀은 `WM_LBUTTONDOWN`을 아예 전달하지 않는다** → 드래그는 불투명 본문에서 시작해야 한다(D4-c). ② 게임이 `ClipCursor`를 걸면 드래그 범위가 제한된다 |
...
- **크기 조절**: 이동 모드 중에만 보이는 커스텀 그립. **그립은 도색된 본문 안쪽으로 들여 배치한다** — 둥근 모서리의 우하단은 투명 픽셀이라 클릭이 닿지 않는다. 규칙으로 승격: **이동 모드의 모든 어피던스는 불투명 픽셀 위에 있어야 한다.**
...
- **통과 메커니즘은 두 가지가 공존한다.** `AllowsTransparency=True`에서 완전 투명 픽셀은 확장 스타일과 무관하게 클릭을 통과시킨다. 이동 모드 중에도 그러하므로 D4-c의 불투명 픽셀 규칙이 나온다.
...
| 표면 | 스타일 | 입력 | 수명 |
|---|---|---|---|
| 오버레이 | `Topmost` + `TRANSPARENT` + `NOACTIVATE` + `AllowsTransparency`, 작업 표시줄 없음 | **없음** (이동 모드 제외) | 앱 수명 전체 |
| 설정 창 | 일반 창, 활성화 가능, 작업 표시줄 표시, **`Owner` 미지정** | 전체 | 열고 닫힘 반복 |
...
| **`Owner` 금지** | 【측정】 `Owner`를 오버레이로 지정하면 exstyle `0x00040108` — `WS_EX_TOPMOST`가 전파되는데 `Topmost` 속성은 false로 읽힌다. 소스 어디에도 `Topmost=true`가 없는 채로 설정 창이 게임 위에 뜬다 |
...
| 요소 | 결정 |
|---|---|
| 채널 | `RegisterWindowMessage`로 앱 고유 메시지를 등록하고, 메시지 전용 창 또는 오버레이 HWND에서 처리한다. **펌프 시작 전에 도착한 신호도 큐에 남으므로** 순서 문제가 사라진다 |
...
| `PollingStopped` | 하트비트 정체(D20) | 라운드 재개 | **배너** — 중단 사실과 마지막 시도 시각 | 로그 폴더 열기, 재시작 안내 | **오류** |
...
| `window.x` / `.y` | double | 100 / 100 | **하나의 작업영역에 완전히 포함**되어야 한다 (3판의 "교차"에서 강화 — D19) | 기본 위치로 클램프 |
...
| FR-07-1 | 모든 문자열 ID 조회 | `Localization` + 전 사용처(**트레이 툴팁·메뉴 포함**) | S2 |
...
| FR-08-2 | 트레이에서 여는 일반 설정 창 | `Shell`(transient 창·뷰모델, **`Owner` 금지** D18-b) + `SettingsViewModel` | S3 |
...
| NFR-01 | 유휴 CPU 무시 가능 | `Polling`(UI 스레드 밖, §3.2) + `Shell`(히트테스트 타이머 없음) + 타이머 2개 유지(§3.3, 하트비트는 기존 타이머 재사용) | S2, S3 |
...
41개 ID 전부 등재. `FR-04-2`·`NFR-04`는 소유 모듈이 없는 순수 정책, `FR-02-1`·`FR-02-2`·`FR-04-1`은 모듈이 아니라 **구조적 제약**으로 성립한다.
...
| **Q5** | **`AllowsTransparency` 재평가** — "소프트웨어 렌더링" 서술은 Windows 7 이전 것이다. 실제 비용은 레이어드 창 present 경로이며 유휴 시 리페인트가 없으므로 **NFR-01은 위험 축이 아니다.** 진짜 비용은 ① **ClearType 서브픽셀 안티에일리어싱 비활성화 → 흐린 글자** (숫자와 소수점을 읽는 것이 존재 이유인 앱에서 기능적 손실), ② 자식 HWND 미렌더, ③ 면적 비례 리페인트 | 중 | S3 측정: 스냅샷 갱신 시 프레임 시간, **글자 가독성**. 가독성이 문제면 해법은 `AllowsTransparency`를 끄는 것이 아니라 **텍스트 뒤 불투명 배경 + `SetLayeredWindowAttributes`** |
...
| # | 항목 | 성격 | 다음 행동 |
|---|---|---|---|
| **Q17** | **설정 창 활성화가 게임의 포그라운드 잠금을 이길 수 있는가.** 3판은 `SetForegroundWindow`가 사라졌다고 보고 관련 위험을 종결했으나, D18-b의 `Activate()`가 바로 그 호출이며 D18-d의 **교차 프로세스** 경우는 더 어렵다. 두 검토자가 독립적으로 지적했고, **앱의 유일한 진입점을 좌우한다** | **높음** | S3 필수 측정. 폴백: 첫 활성화가 성공할 때까지 설정 창을 `Topmost`로 띄운다 |
