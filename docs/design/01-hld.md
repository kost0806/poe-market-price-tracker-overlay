...
| | |
|---|---|
| 문서 상태 | **개정 8판** — 7판(S1 동결) 위에 REQUIREMENTS 4판의 세 결정(변동률 미표시 · 폴링 기본 15분 · 조건부 요청)을 반영 |
| 작성일 | 2026-08-15 |
| 상위 문서 | `docs/REQUIREMENTS.md` **개정 4판**, `docs/design/00-api-contract.md` (**데이터 계약 — 구속력 있음**), `docs/design/00-shell-measurements.md` (**Win32·렌더링 실측 — 구속력 있음**) |
...
### 0.0 개정 8판 — 실측이 뒤집은 표시 하나를 빼고, 이용 지침을 설계에 넣는다

세 가지다. **어느 것도 새 기능이 아니다 — 두 개는 잘못된 표시를 지우는 일이고, 하나는 남의 서버에 대한 예의다.**

| # | 변경 | 근거 |
|---|---|---|
| 1 | **변동률 칸 삭제** (§6.3, D25, 모듈 책임표) | `00-api-contract.md` §3.2 실측 — 창이 약 7일이고 기준 통화가 `maxVolumeCurrency`다. 세션 중 움직이지 않는 데다, 표시 통화를 고정한 행에서는 부호가 갈린다 |
| 2 | **폴링 기본값 5분 → 15분** (D11, §3.4 타이머표, 추적표 FR-03-1) | 같은 문서 §7.3 — poe.ninja가 원본 갱신 주기를 «약 15분»으로 공표했다. 하한 5분은 유지 |
| 3 | **조건부 요청 신설** (D24, 추적표 NFR-02) | 같은 문서 §7.3·§7.4 — 지침이 요구하고, `304`·0바이트가 실측됐다 |

7판이 「S1 동결」이었다는 사실은 지우지 않는다. **동결은 합의를 뜻하지 사실을 이기지 못한다** — 이 저장소에서 실측이 설계 주장을 뒤집은 사례가 또 한 번 늘었다.

### 0.1 개정 7판 — 최종 정합성 마감 지시서 반영. 구현 가능한 상태로 닫는다
...
### 0.2 개정 6판 — S3(`03-lld-shell.md` 제4판) §13의 검증된 42행 개정 목록을 반영
...
### 0.4 개정 4판 — 측정으로 확정하고, 표시 전용화가 남긴 구멍을 메웠다
...
| 11 | 서킷 브레이커 삭제, **429/`Retry-After`** 추가 | 15분에 카테고리당 1회는 표본창을 못 채움 (8판 전에는 5분이었고, 결론은 더 강해졌다) |
...
graph LR
    ...
    APP -->|HTTPS GET, 카테고리 단위 15분 주기, If-None-Match| NINJA
...
| `Pricing` | **순수 계산 전부** — 표시통화 결정, 디바인 환산, 역수 판정, FR-04-4 5행 서식, 상대 시각, vintage 산출. **변동률은 계산하지 않는다**(FR-04-1, §6.3) | `Domain`, `Localization` |
...
| 타이머 | 주기 | 소유 | 유휴 비용 |
|---|---|---|---|
| 폴링 `PeriodicTimer` | 기본 15분, 하한 5분 (`TimeProvider` 주입) | `Polling` | 실질 0. 대기 중 스레드 점유 없음 |
...
| 기본 경로 | **카탈로그 + 캐시 교차 검색** 【개정 2026-08-17】. `Store`는 지금까지처럼 캐시 두 슬롯을 뒤지고, **`Presentation`이 동봉된 아이템 카탈로그(`00-api-contract.md` §6.8)를 함께 뒤져 병합**한다. 결과에 **카테고리 라벨**을 붙인다. 네트워크 비용은 여전히 0이다 — 카탈로그는 파일이다 |
| 왜 `Store`가 아니라 `Presentation`인가 | 카탈로그 검색은 **지역화된 이름으로** 매칭해야 하는데 `Store`는 `Localization`을 참조하지 않는다(S2 1.2) — 캐시 검색이 이미 그 이유로 매칭 술어를 주입받고 있다(`SearchOptions.ExtraMatch`). 게다가 카탈로그에는 `(league, dataEpoch)` 태그가 붙지 않는다: 리그와 무관한 **파일**이므로 `Store`의 슬롯 규율에 들어갈 것이 아니다. `Store`에 넣으면 이 모듈에 없던 파일 I/O가 생긴다 |
| 시세가 없는 적중 | 카탈로그에만 있는 아이템도 **결과로 낸다.** 값 자리에 `시세 없음`을 적고, 관심목록 추가는 그대로 동작한다 — 추가가 그 카테고리를 1회 조회한다(이미 있던 경로다) |
| 보조 경로 | 카테고리 드롭다운 브라우징은 유지한다. 아직 받지 않은 카테고리를 고르면 **그 하나만** 조회한다 |
| 빈 결과 | "캐시에 없음"과 "존재하지 않음"을 **구별해 말한다.** 카탈로그에도 없으면 그것은 이 리그에 없는 이름이다 |
| **초판이 틀린 곳** | 초판은 "캐시 교차 검색"만 두었고, 그것은 **앱이 아이템 목록을 하나도 동봉하지 않던 시점에는 참인 전제**였다. 사전과 카탈로그가 들어온 뒤(2026-08-16~17) 이 결정을 다시 보지 않았고, 그 사이 사용자에게 보이는 증상은 "갑충석은 지원하지 않는 앱"이었다. **전제가 바뀌면 그 위의 결정을 다시 여는 것까지가 결정의 일부다** |
...
- `refreshIntervalMinutes`의 **하한은 5분, 기본값은 15분**이다(8판, REQUIREMENTS 4판 FR-03-1). NFR-02가 "카테고리 단위 호출 + 최소 5분 주기"를 무조건으로 규정하므로 FR-03-1의 "변경 가능"과 양립하는 유일한 해석은 **하한 아래로는 못 내려간다**이다. 상한 60분.
- 기본값이 5분에서 15분으로 올라간 근거는 poe.ninja가 공표한 원본 갱신 주기다 — PoE 1 overview는 **약 15분마다** 갱신된다(`00-api-contract.md` §7.3). 5분 기본값은 같은 값을 세 번 받아 온다.
...
### D24. 조건부 요청 — `ETag`는 **가지고 있는 스냅샷과 함께 다닌다** (신설, NFR-02)

poe.ninja의 이용 지침이 조건부 요청을 명시적으로 요구하고(*"Use conditional requests and respect the cache headers; do not bypass caching"*), 실측에서 `If-None-Match`가 `304`·0바이트로 응답된다(`00-api-contract.md` §7.3·§7.4).

| 결정 | 내용 |
|---|---|
| **`ETag`의 자리** | **`CategorySnapshot`이 든다.** `Market`에 사전을 따로 두지 않는다 |
| 왜 사전이 아닌가 | 사전을 두면 **무효화해야 할 상태가 둘로 갈라진다** — 리그 교체(D9의 epoch)나 스냅샷 폐기 때 둘을 함께 비워야 하고, 한쪽만 비면 «데이터는 없는데 검증자는 있는» 상태에서 `304`가 와서 **아무 값도 남지 않는다.** 검증자를 그것이 검증하는 데이터 옆에 두면 그 상태가 **구조적으로 생길 수 없다** |
| 요청 | `Polling`이 그 카테고리의 **현재 스냅샷을 그대로 넘긴다.** `Market`은 거기 실린 `ETag`를 `If-None-Match`로 싣는다. 스냅샷이 없으면 헤더도 없다 — 리그가 바뀐 라운드는 기준 스냅샷 자체가 비어 있으므로(§4.4의 `baseline`) 자동으로 무조건부 요청이 된다 |
| 응답 `304` | **성공이다.** `Market`은 넘겨받은 스냅샷을 `FetchedAt`만 지금으로 바꿔 돌려준다 — 「우리가 든 값이 여전히 poe.ninja의 현재 값」이라는 서버의 확인이기 때문이다. 실패로 흘리면 연속 실패 카운터가 오르고 백오프·쿨다운이 걸린다 |
| 갱신 시각 | 위에 따라 **갱신된다.** 그러지 않으면 시세가 조용한 밤새 노후 표식이 뜬다 |
| 스냅샷을 안 주고 `304`가 오면 | 있을 수 없는 응답이다(헤더를 안 보냈으므로). **실패로 처리한다** — 조용히 넘기면 값 없는 성공이 된다 |
| 사용자 개시 조회 | 스냅샷을 넘기지 않는다. 「지금 조회」는 재검증이 아니라 조회다 |

세부(시그니처, 결과 형태, 로그 코드)는 S2 §5.11·§7.3과 S4가 확정한다.

### D25. 변동률 표시 제거 (신설, FR-04-1)

§6.3이 근거를 든다. 여기서는 **모듈 경계에 미치는 영향**만 적는다: `Pricing`에서 변동 계산이 사라지고(모듈 책임표 §2), `Presentation`의 행 뷰모델에서 변동 필드가 사라지며, `Domain`의 `TotalChangePercent`와 `Market`의 파싱은 **남는다**. 거래량과 같은 취급이다 — 파싱을 지우면 응답 형식 변화 감지(G3)가 함께 사라진다.
...
│                             1d당 3,040개      │ ← 시세 행 영역
│ ▣ Essence of Horror                    43.5c  │    유일하게 잘릴 수 있는 구간
...
**변동률 칸은 없다 (REQUIREMENTS 4판, FR-04-1).**

7판까지 이 자리에는 `▲30.5%` 형태의 변동률 칸과 글리프·방향·색 규칙표가 있었다. `00-api-contract.md` §3.2의 실측이 그 표를 무효로 만들었다.

| 실측 | 무엇이 무너졌나 |
|---|---|
| `totalChange`의 창은 **약 7일**이다 (§3.2.1) | 「상시 감시」 화면에서 세션 내내 움직이지 않는 수다. D2가 «현재가+변동률로 충분»이라고 적었을 때 상정한 것은 지금 움직이는 수였다 |
| 기준 통화가 `core.primary`가 아니라 **그 행의 `maxVolumeCurrency`** 다 (§3.2.2) | 표시 통화를 `카오스`·`디바인`으로 **고정한 행에서는 가격과 변동률의 기준이 다르다.** 두 통화가 서로 반대로 움직인 창에서는 부호가 갈린다 — 색까지 반대로 칠해진다 |

**따라서 `Pricing`은 변동률을 계산하지 않고, 행 뷰모델은 변동률을 나르지 않으며, View에는 그 칸이 없다.** `Domain`은 값을 계속 보존한다(거래량과 같은 취급, FR-04-1). 다시 그리기로 한다면 창(7일)과 기준 통화를 **화면에서 밝히는 것**이 선결 조건이다.
...
| FR-03-1 | **기본 15분**, 설정 가능 | `Polling`(`PeriodicTimer`, `Period` 재설정) + `Settings`(5~60 클램프, 기본 15) | S2 |
...
| NFR-02 | 과도한 요청 금지 | `Market`(`NinjaGateway` + **조건부 요청 D24**) + `Polling`(배치·15분 기본·5분 하한·재폴링 디바운스) + `Shell`(단일 인스턴스 가드 D18-d) | S2, S3 |
...
| Q3 | 갱신 주기 하한 | 5분(NFR-02 무조건 규정), 기본 15분(8판). D11 |
