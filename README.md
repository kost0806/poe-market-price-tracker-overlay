...
관심목록에 담은 아이템을 **15분마다**(설정 가능, 최소 5분) 카테고리 단위로 조회해, 한 줄에 하나씩 그린다. poe.ninja가 원본을 약 15분마다 갱신하므로 그보다 자주 물어도 같은 값이 온다. 조회는 `If-None-Match` 조건부 요청이라, 바뀐 것이 없으면 본문이 오지 않는다.

```
[아이콘]  뿔 갑충석 (역병)            359.7c (1.85d)
[아이콘]  혈기생기                      1c당 15.5개
                                        마지막 갱신: 2분 전
```

- **아이콘 · 이름 · 현재가**, 그리고 화면 아래에 마지막 갱신 시각.
- **변동률은 그리지 않는다.** API가 주는 값은 **7일 창**이라 플레이하는 동안 움직이지 않고,
  기준 통화가 그 아이템의 주 거래 통화라서 카오스로 고정해 둔 행에서는 부호까지 어긋난다
  (실측: `docs/design/00-api-contract.md` §3.2).
...
| | 이유 |
|---|---|
| 이력 가격 차트 | API가 절대 가격의 시계열을 주지 않는다. API가 계산해 주는 변동률은 7일 창이라 대체가 되지 못한다 |
...
| 자체 백엔드 경유 | poe.ninja는 데스크톱 앱이 자기 백엔드를 거치기를 **권고**한다. 본인 전용 로컬 빌드에 서버를 두지 않는 대신, 조건부 요청과 15분 주기로 트래픽을 지침보다 더 줄였다 (`docs/design/00-api-contract.md` §7) |
